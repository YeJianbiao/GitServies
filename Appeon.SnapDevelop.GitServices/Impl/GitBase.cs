using Appeon.SnapDevelop.GitServices.Helpers;
using LibGit2Sharp;
using System;
...
{
    internal class GitBase : IPostTestDirectoryRemover, IDisposable
    {
            ...
            private static void SetUpTestEnvironment()
            {
                ...
                {
                    resourcesPath = Path.Combine(Directory.GetParent(new Uri(typeof(GitBase).GetTypeInfo().Assembly.CodeBase).LocalPath).FullName, "Resources");
                }
            ...
            {
                var oldTestRepos = new DirectoryInfo(GitConstants.TemporaryReposPath)
            ...
            {
                var mixedPath = Path.Combine(GitConstants.TemporaryReposPath, "mIxEdCase-" + Path.GetRandomFileName());
            ...
            protected static string Touch(string parent, string file, string content = null, Encoding encoding = null)
            {
                ...
                if (GitConstants.IsRunningOnUnix && newFile)
                {
